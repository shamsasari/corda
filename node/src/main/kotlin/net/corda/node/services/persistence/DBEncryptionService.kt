 ...
 *
 * Storing encryption keys in a database is not secure, and so only use this service if the data being encrypted is also stored unencrypted
 * in the same database.
 *
 * To obfuscate the keys, they are stored wrapped using an AES key derived from the node's legal identity. This is not meant to be secure,
 * it's only meant to reduce the impact of accidental leakage.
 */
// TODO Add support for key expiry
class DBEncryptionService(private val database: CordaPersistence) : EncryptionService, SingletonSerializeAsToken() {
    companion object {
    ...
    fun start(ourIdentity: Party) {
        database.transaction {
            val criteria = session.criteriaBuilder.createQuery(DBEncryptionKey::class.java)
            criteria.select(criteria.from(DBEncryptionKey::class.java))
    ...
    override fun encrypt(plaintext: ByteArray): ByteArray {
        ...
        val buffer = ByteBuffer.allocate(UUID_BYTES + ciphertext.size)
        // Prepend the key ID to the returned ciphertext. It's OK that this is not included as authenticated data in the AES encryption
        // because changing this value will lead to either an non-existent key or an another key which will not be able decrypt the ciphertext.
    ...
    override fun decrypt(ciphertext: ByteArray): ByteArray {
        ...
        return Encryption.aesDecrypt(aesKey, buffer.copyBytes())
    }
    ...
    /**
     * Derive the key encryption key from the the node's identity and the persisted key's ID.
     */
    private fun createKek(ourIdentity: Party, keyId: UUID): Key {
        ...
        return SecretKeySpec(digest.digest(), 0, AES_KEY_SIZE_BYTES, "AES")
    }


    @Entity
    @Table(name = "${NODE_DATABASE_PREFIX}encryption_keys")
    class DBEncryptionKey(
            @Id
            @Type(type = "uuid-char")
