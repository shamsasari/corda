...
class ExternalVerifier(
    ...
    private fun initialise() {
        ...
        val appClassLoader = createAppClassLoader()

        // Then use the initialisation message to create the valid serialization context
    ...
    private fun verifyTransaction(request: VerificationRequest) {
        val verificationContext = ExternalVerificationContext(this, request.inputsAndReferences)
        val transaction = request.transaction
        val result = try {
            when (transaction) {
                is WireTransaction -> transaction.verifyInternal(verificationContext)
                is ContractUpgradeWireTransaction -> TODO()
                else -> throw IllegalArgumentException("${transaction.javaClass.name} is not supported")
            }
            log.info("Transaction ${transaction.id} verified")
            VerificationSuccess
        } catch (t: Throwable) {
            log.info("Transaction ${transaction.id} failed to verify", t)
    ...
    private inline fun <reified T : Any> request(request: Any): T {
        log.info("Sending request to node: $request")
